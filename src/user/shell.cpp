...
auto Shell::Prepare_Stdio_For_Single_Command(Command& command) const -> std::pair<bool, std::string> {
	...
	if (command.redirect_type == RedirectType::None) {
		command.Set_Pipe_File_Descriptor(std_in, std_out);
		return {true, ""};
	}

	// Zkusime nastavit vstupni soubor
	if (command.redirect_type == RedirectType::Both || command.redirect_type == RedirectType::FromFile) {
		if (const auto success = kiv_os_rtl::Open_File(command_fd_in, command.source_file,
	...
	if (command.redirect_type == RedirectType::Both || command.redirect_type == RedirectType::ToFile) {
		if (const auto success = kiv_os_rtl::Open_File(command_fd_out, command.target_file,
		                                               kiv_os::NOpen_File::fmOpen_Always); !success) {
			Close_Command_File_Descriptors(command);
...
auto Shell::Prepare_Stdio_For_First_Command(Command& command,
	...
	if (const auto success = kiv_os_rtl::Create_Pipe(command_std_out, next_command_std_in); !success) {
		Close_Command_File_Descriptors(command); // Zavreme file descriptory, protoze selhalo
	...
	command.Set_Pipe_File_Descriptor(command_std_in, command_std_out);
...
auto Shell::Prepare_Stdio_For_Commands(std::vector<Command>& commands) const -> std::pair<bool, std::string> {
	...
	for (size_t i = 1; i < commands.size() - 1; i += 1) {
		...
		if (command.redirect_type != RedirectType::None) {
			Close_Command_List_File_Descriptors(commands, i); // Zavreme vsechny file descriptory predchozich prikazu
		...
		command.Set_Pipe_File_Descriptor(command_fd_in, command_fd_out);
	...
	if (last_command.redirect_type == RedirectType::Both || last_command.redirect_type == RedirectType::FromFile) {
		Close_Command_List_File_Descriptors(commands, commands.size() - 1);
	...
	last_command.Set_Pipe_File_Descriptor(command_fd_in, command_fd_out);
	// Nastavime poslednimu souboru file descriptory

	return {true, ""};
}

void Shell::Close_Command_File_Descriptors(const Command& command) const {
...
void Shell::Close_Command_List_File_Descriptors(const std::vector<Command>& commands, const size_t count) const {
...
void Shell::Run_Commands(std::vector<Command>& commands) {
	...
	for (auto i = 0; i < commands.size(); i += 1) {
		...
		if (!success) {
			...
			Close_Command_List_File_Descriptors(commands, i, commands.size() - 1);
			return;
		}
